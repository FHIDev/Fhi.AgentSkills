...
| Problem | Håndtering |
|---|---|
| `check-version.mjs` feiler | Manuell fremgangsmåte: (1) Les versjon fra `maintenance/designsystem/.oppdater-state.json` eller `<!-- Basert på ... -->` i `plugins/designsystem/skills/designsystem/SKILL.md`. (2) Slå opp latest: `npm view {pakkenavn} version` eller `https://registry.npmjs.org/{pakkenavn}/latest`. (3) Mellomliggende minors: `npm view {pakkenavn} versions --json`, filtrer bort pre-releases, finn høyeste patch per minor mellom forrige og ny latest (begge ekskl.). |
